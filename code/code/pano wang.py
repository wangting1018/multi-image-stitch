...
class Stitch:
        ...
        def rightshift(self):
                for each in self.right_list:
                        #cv2.imshow("leftp0", self.leftImage)
                        cv2.waitKey()
                        leftImage=self.change_size(self.leftImage)
                        cv2.imshow("leftp", leftImage)
                        cv2.waitKey()
                        ...
                        dsize = (each.shape[1]+self.leftImage.shape[1], each.shape[0]+self.leftImage.shape[0])
                        tmp = cv2.warpPerspective(each, H, dsize)
                        #cv2.imshow("tp", tmp)
                        cv2.waitKey()
                        tmp[:leftImage.shape[0], :leftImage.shape[1]]=leftImage
                ...
                for i in range(0,x):
                        for j in range(0,y):
                                if((leftImage[j,i]==np.array([0,0]))):
                                   ...
                                   top=max(edges_y)
                                   height=top-bottom
                                   leftImage=leftImage[left:left+width-50,bottom:bottom+height-50]
                        
                print(edges_x)                  
                                
                #cv2.imshow("pre1_picture", self.leftImage)
                """
                for k in range(2500,x):
                        if (leftImage[int(leftImage.shape[0]/2),k][0]==0):
                                leftImage=leftImage[:,0:k-20]
                                break
...
if __name__ == '__main__':
        ...
        s.rightshift()
        print ("done")
        cv2.imwrite("ice-7.jpg", s.leftImage)
        print ("image written")
        cv2.imshow("tmp", s.leftImage)
        cv2.waitKey()
